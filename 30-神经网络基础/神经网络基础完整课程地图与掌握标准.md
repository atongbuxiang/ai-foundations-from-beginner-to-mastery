...
| 现行教学迁移 | 8 / 64 | NN-01—08 已由[[neural_network_foundations_teaching_contract_audit.py]]复核课程位置、两遍路线、问题链、对象账本、贯穿算例与公式七问 |
| 待迁移节点 | 56 / 64 | NN-09—64 保留既有正文、题解与图，但尚未按现行教学合同回归 |
...
| 已通过分卷 | 0 | 尚无真实学习证据 |
| 下一材料施工点 | NN-09—12 | 进入 30.2 计算图、拓扑执行、链式法则与 reverse-mode 的前半卷迁移 |

当前状态是 **8/64 教学迁移完成、56/64 待迁移、30.1 材料门 1/8 `regression-passed`、个人 `not-attempted`**。旧 `NN-CUM-01` 的存在只说明历史材料组成，不应跳过现行教学迁移直接要求学习者参加总测验。
