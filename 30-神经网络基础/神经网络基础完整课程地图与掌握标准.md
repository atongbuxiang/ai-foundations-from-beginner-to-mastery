...
| 现行教学迁移 | 16 / 64 | NN-01—16 已由[[neural_network_foundations_teaching_contract_audit.py]]复核课程位置、两遍路线、问题链、对象账本、贯穿算例与公式七问 |
| 待迁移节点 | 48 / 64 | NN-17—64 保留既有正文、题解与图，但尚未按现行教学合同回归 |
| 分卷材料回归 | 2 / 8 | 30.1—30.2 的节点、题解、正式图、来源与共享 fixture 已通过；个人仍为 `not-attempted` |
| 旧章节累计门 | composed | NN-CUM-01 题卷、独立解答与计算复现门已组成；须在节点迁移与分卷门升级后重新审计 |
| 已通过分卷 | 0 | 尚无真实学习证据 |
| 下一材料施工点 | NN-17—20 | 进入 30.3 激活函数、门控与非线性前半卷 |

当前状态是 **16/64 教学迁移完成、48/64 待迁移、30.1—30.2 材料门 2/8 `regression-passed`、个人 `not-attempted`**。旧 `NN-CUM-01` 的存在只说明历史材料组成，不应跳过现行教学迁移直接要求学习者参加总测验。
