...
> [!note] 课程位置
> NUM-14 已给出确定性 SVD 与谱范数路线，NUM-19 又说明大规模稀疏/外存矩阵的 data pass 往往比 FLOP 更贵；本章用随机探针把“先找值域、再做小 SVD”变成可扩展方案。它也是 10.8 的收束节点：最终交付必须同时包含确定性线性代数、随机失败预算、独立后验检查和系统成本。

> [!tip] 建议两遍阅读
> 第一遍只跟随一个固定 Rademacher sketch，手算 $Y=A\Omega$、投影补空间和谱范数误差，再比较 $p=0$ 与一次过采样；第二遍再进入 Gaussian 期望/尾界、幂步、single-pass、Nyström/CUR/ID 与验证探针。先分清 range error 和最终 rank-$k$ error，再谈“随机 SVD 精度”。

## 本章的推导问题链

1. 最佳 rank-$2$ 误差由哪一个奇异值给出？
2. 两列 Rademacher 探针经 $A$ 作用后张成什么二维值域？
3. 如何不用完整 SVD，仅凭补空间单位向量精确算出 range residual？
4. 为什么本次 $p=0$ sketch 的误差大于最优 rank-$2$ 误差？
5. 在三维模型中加一个过采样方向，为何能令 range error 为零而 final rank-$2$ error 仍非零？
6. 幂步怎样把尾部/主子空间比例从 $\sigma_3/\sigma_2$ 变成其 $2q+1$ 次幂？
7. 为什么验证探针必须独立于构造 sketch，并显式报告失败概率？

## 贯穿算例：一次固定随机实现究竟捕获了什么
...
目标秩取 $k=2$。由 Eckart–Young–Mirsky，最佳 rank-$2$ 谱范数误差是

$$
E_\star=\sigma_3=\sqrt2-1\approx0.4142.
$$

下面把一次 Rademacher 随机抽样的结果固定下来做手算；“固定”只表示条件在这次 seed/realization 上分析，不把它伪装成确定性最坏保证。
...
| $k$ | 目标 rank | $2$ |
| $p$ | 过采样维数 | 先取 $0$，再比较 $1$ |
| $\ell=k+p$ | sketch 宽度 | $2$ 或 $3$ |
| $\Omega$ | 随机测试矩阵 | 固定 Rademacher realization |
| $Y=A\Omega$ | sketch 后样本 | 近似值域的原始基 |
| $Q=\operatorname{orth}(Y)$ | 标准正交值域基 | 定义投影 $P=QQ^T$ |
| $E_{\rm range}$ | $\|(I-P)A\|_2$ | 是否捕获重要值域 |
| $E_k$ | 最终 rank-$k$ 误差 | 还包含小空间截断 |

### 第一步：$p=0$ 时固定两列 Rademacher sketch
...
两列线性无关，因此 $\operatorname{range}(Y_0)$ 是二维平面。无需先写出繁复的 QR 系数，只需找到它的单位法向量：

$$
w=\frac1{\sqrt{46}}(3,-6,-1)^T.
$$

直接检查 $w^TY_0=0$，故若 $Q_0$ 是 $Y_0$ 的任意标准正交基，

$$
I-Q_0Q_0^T=ww^T.
$$

### 第二步：用一维补空间精确计算 range error
...
因为 $\ell=k=2$，小空间中已没有额外维度可供“先保留再截断”，最终 rank-$2$ 近似就是 $Q_0Q_0^TA$，误差同样为 $3/\sqrt{23}$，大于最优值 $\sqrt2-1$。

### 第三步：过采样把 range capture 与最终截断分开

再增加一列，取固定 realization
...
$A$ 也可逆，所以 $Y_1=A\Omega_1$ 满秩，$Q_1$ 张成整个 $\mathbb R^3$。于是

$$
E_{\rm range}=0.
$$

但目标仍是 rank $2$；对小矩阵 $B=Q_1^TA$ 做确定性 rank-$2$ 截断后，最终误差回到

$$
E_2=\sigma_3=\sqrt2-1,
$$

而不是零。这是“值域已经完整捕获”和“最终必须丢掉一个奇异方向”之间最清楚的区别。
...
$$
\left(3-2\sqrt2\right)^{2q+1}.
$$

例如 $q=1$ 时约为 $0.00505$。代价是额外 $A^T/A$ pass、正交化、通信与舍入风险；不能只报告“误差更小”而隐藏数据遍历。

### 第五步：构造 sketch 不能给自己签发证书

对 fresh 且独立的 Gaussian probes $g_j$，计算
...
所以每个 probe 都在一维 residual 方向 $w$ 上。多个独立 $\|z_j\|$ 的最大值配合明确概率常数，可给 $\|(I-Q_0Q_0^T)A\|_2$ 后验上界；若重复使用 $\Omega_0$ 中的列，验证量会与构造误差相关，失败概率声明不再成立。

### 核心公式七问：$E_{\rm range}=\|(I-QQ^T)A\|$

1. **它测量什么？** 所有单位输入中，$A$ 输出落在 $\operatorname{range}(Q)$ 之外的最坏剩余。
2. **为何不等于最终 rank-$k$ error？** 当 $\ell>k$ 时，还要在捕获的 $\ell$ 维值域中截断，第二层会重新产生误差。
3. **随机性在哪里？** 给定 $\Omega$ 后，$Y,Q$ 与误差都是确定的；概率只来自抽取 sketch 的分布。
4. **过采样做什么？** 为主子空间提供冗余观测，降低 $\Omega_1$ 近奇异和漏掉重要方向的尾部风险。
5. **幂步做什么？** 把奇异值权重提高到 $2q+1$ 次方，用更多 pass 放大谱分离；它不改变最终目标秩。
6. **为什么需要独立验证？** 构造 sketch 已经被算法用于选择 $Q$，同一随机信息不能再当作无偏/独立的失败检测。
7. **AI 中如何验收？** PCA、LoRA 与激活压缩应同时给矩阵范数误差、子空间角、下游指标、seed/p/q、data pass、内存和失败概率。

> [!warning] 教学模型边界
> 三维中 $p=1$ 已让 $\ell=n$，所以 range error 必然为零；大规模算法绝不会靠“过采样到全维”获得效率。固定 Rademacher realization 只用于可复核手算，不能代替对 sketch 分布的期望/尾界，也不能把一次成功外推成统一成功率。

> [!success] 第一遍停靠线
> 应能从 $Y_0$ 找到 $w=(3,-6,-1)^T/\sqrt{46}$，推出 $E_{\rm range}=3/\sqrt{23}$，再解释 $p=1$ 时 range error 为零而 final rank-$2$ error 仍为 $\sqrt2-1$。还要能说明 $p$、$q$ 与独立 validation probes 分别控制冗余、谱分离和本次运行的后验风险。
