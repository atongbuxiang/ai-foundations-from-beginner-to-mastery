...
> [!note] 课程位置
> NUM-05 已说明 reduction path 怎样丢失小量；本章把同一现象放进 Gaussian elimination，区分问题条件性、pivot choice、factor growth 与最终 BERR/FERR。NUM-07 将复用这里的近似 factors 解误差方程，NUM-08 则展示不用一般消元乘子、改用正交变换制造零的路线。

> [!tip] 建议两遍阅读
> 第一遍只手算下面的 $2\times2$ 系统：分别完成不选主元和部分选主元的消元、回代、残差与前向误差；第二遍再进入一般 $PA=LU$、growth factor、equilibration、结构化求解和 LAPACK 证书。若不能解释“条件良好仍可能被无主元算法算坏”，先不要记 GEPP 的一般界。

## 本章的推导问题链

1. 先检查原问题是否病态，避免把算法失败误判成 condition number 大；
2. 在不选主元时计算 multiplier，观察小 pivot 怎样制造巨大中间量；
3. 沿实际舍入路径完成消元和回代，确认输出怎样失去一个分量；
4. 交换行后重新分解，比较 multiplier 与 growth；
5. 回到原始 $A,b$ 重算 true residual，再分别评价 forward error 与 BERR；
6. 最后才把二维机制推广到一般 partial/rook/complete/threshold pivoting。

## 贯穿算例：一个条件良好的系统为什么仍需要选主元
...
$$
x_{\star,1}=\frac1{1-\varepsilon},
\qquad
x_{\star,2}=\frac{1-2\varepsilon}{1-\varepsilon}.
$$

计算仍在四位十进制教学算术 $\mathbb F_{10,4}$ 中进行。

### 符号与对象账本

| 对象 | 本例中的值 | 解释 |
|---|---|---|
| small pivot | $a_{11}=\varepsilon=10^{-8}$ | 非零不等于数值上适合做 pivot |
| 无主元 multiplier | $m=1/\varepsilon=10^8$ | 制造巨大中间量和分辨率损失 |
| 置换矩阵 | $P=\begin{bmatrix}0&1\\1&0\end{bmatrix}$ | 把模最大的候选 pivot 移到首行 |
| 选主元 multiplier | $\widetilde m=\varepsilon$ | 绝对值不超过 $1$ |
| 真解 | $x_\star$ | 原始 $A_\varepsilon x=b$ 的精确答案 |
| 计算解 | $\widehat x$ | 某条具体消元/回代路径的输出 |
| true residual | $r=b-A_\varepsilon\widehat x$ | 必须用原始数据重算 |
...
A_\varepsilon^{-1}
=\frac1{1-\varepsilon}
\begin{bmatrix}-1&1\\1&-\varepsilon\end{bmatrix}.
$$
...
=\|A_\varepsilon\|_\infty\|A_\varepsilon^{-1}\|_\infty
=\frac4{1-\varepsilon}
\approx4.
$$

这是条件良好的问题。如果算法给出 $O(1)$ 前向误差，主要责任不能推给固有敏感性。

### 第二步：不选主元时，两个不同的数舍入成同一个数

第一步 multiplier 为

$$
m=\operatorname{fl}(1/\varepsilon)=10^8.
$$

消元后的第二个 pivot 和右端分别是
...
在 $\mathbb F_{10,4}$ 中二者都舍入为 $-1.000\times10^8$，于是回代得到

$$
\widehat x_2=1,
\qquad
\widehat x_1
=\operatorname{fl}\!\left(\frac{1-\widehat x_2}{\varepsilon}\right)
=0.
$$
...
$$
\widehat x_{\rm no\ pivot}=(0,1)^\mathsf T.
$$

其相对无穷范数前向误差恰为

$$
\frac{\|x_\star-\widehat x_{\rm no\ pivot}\|_\infty}{\|x_\star\|_\infty}=1.
$$

### 第三步：部分选主元把 multiplier 从 $10^8$ 变成 $10^{-8}$
...
$$
L=\begin{bmatrix}1&0\\\varepsilon&1\end{bmatrix},
\qquad
U=\begin{bmatrix}1&1\\0&1-\varepsilon\end{bmatrix}.
$$

四位算术把 $1-\varepsilon$ 和 $1-2\varepsilon$ 都舍入为 $1$，回代得到

$$
\widehat x_{\rm pivot}=(1,1)^\mathsf T.
$$

它并非精确真解，但相对无穷范数前向误差只有

$$
\frac{\|x_\star-\widehat x_{\rm pivot}\|_\infty}{\|x_\star\|_\infty}
=\varepsilon=10^{-8}.
$$

本例中 $|l_{21}|=\varepsilon$，且 $U$ 没有产生超过原矩阵最大元素的增长；这解释了改进来自执行路径，而不是问题被换成了条件更好的问题。

### 第四步：用原始方程验收，而不是只看 factorization success

对失败解

$$
r_{\rm no\ pivot}
=b-A_\varepsilon(0,1)^\mathsf T
=(0,1)^\mathsf T,
$$
...
$$
r_{\rm pivot}
=b-A_\varepsilon(1,1)^\mathsf T
=(-\varepsilon,0)^\mathsf T.
$$
...
$$
\operatorname{BERR}
=\max_i\frac{|r_i|}{(|A_\varepsilon||\widehat x|+|b|)_i}
=\frac{\varepsilon}{2+\varepsilon}
\approx5\times10^{-9}.
$$

这才形成完整证据：问题条件良好、pivot multiplier 受控、true residual 小、BERR 小且实际前向误差为 $10^{-8}$。

### 核心公式七问：为什么要写 $PA=LU$

1. **$P$ 做什么？** 只重新排列方程，不改变精确解集合；同时改变浮点消元路径。
2. **为何选列内最大候选？** 使 partial-pivot multiplier 满足 $|l_{ik}|\le1$，避免本例的 $10^8$ 放大。
3. **这足以证明稳定吗？** 不足；多步 $U$ 仍可能出现大的 element growth。
4. **为什么不显式求逆？** 分解加三角求解更省成本、误差分析更清楚，也可复用多右端。
5. **BERR 在哪一层？** 它评价最终 $\widehat x$ 对原始 $A,b$ 的邻近问题解释，不是因子残差的别名。
6. **FERR 从哪来？** 还需把 BERR 与匹配的条件估计结合；病态问题中 BERR 小仍可能 FERR 大。
7. **AI 中如何用？** 隐式层、K-FAC/二阶子问题和 covariance solve 都应报告 pivot/structure、residual、BERR、condition estimate 与 fallback。

> [!warning] 教学模型边界
> 本例只隔离 single small pivot；它不代表 GEPP 在所有矩阵上 growth 都为 $1$。稀疏系统还要兼顾 fill，结构化矩阵可能禁止任意行列交换，近秩亏系统可能应改用 QR/SVD。四位十进制结果也不是某个 LAPACK kernel 的逐位预测。

> [!success] 第一遍停靠线
> 应能不看答案算出 $\kappa_\infty=4/(1-\varepsilon)$、无主元 multiplier $10^8$、失败解 $(0,1)^T$、选主元 multiplier $10^{-8}$、改进解 $(1,1)^T$、前向误差 $1$ 与 $10^{-8}$，以及选主元解的 BERR $=\varepsilon/(2+\varepsilon)$。
