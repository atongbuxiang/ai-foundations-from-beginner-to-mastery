...
> [!note] 课程位置
> NUM-08 已经给出稳定 QR 的底层正交变换；本章把它用于第一个真正的算法选择问题：正规方程、QR 和 SVD 在精确算术中可以返回同一参数，却保留不同的数值信息。本章产生的正规矩阵 $G=A^TA$ 将成为 NUM-10—12 的统一谱对象。

> [!tip] 建议两遍阅读
> 第一遍只追踪下面的 $4\times3$ 设计矩阵，确认投影残差、三个奇异方向和条件数平方；第二遍再进入 QRCP、COD、TSVD、ridge、欠定系统和加权最小二乘。先能说清“拟合残差小”和“参数弱方向准确”为什么不同，再比较库函数。

## 本章的推导问题链

1. 怎样构造一个精确解、非零残差和奇异值都已知的超定问题？
2. 为什么 $A^Tr_\star=0$ 同时是投影几何和一阶驻点条件？
3. 形成 $G=A^TA$ 后，奇异值为什么变成平方？
4. 弱方向在 $A$ 中只缩小 $4$ 倍，为什么在 $G$ 中缩小 $16$ 倍？
5. 正规方程、Householder QR 与 SVD 各自交付什么证书？
6. 若数据接近秩亏，应改变求解器，还是连目标定义也要通过截断/正则化改变？

## 贯穿算例：同一个最小二乘问题怎样产生后续整条谱主线
...
\in\mathbb R^{4\times3},
\qquad
x_\star=\begin{bmatrix}1\\-1\\2\end{bmatrix},
\qquad
b=Ax_\star+e_4.
$$

这里 $e_4=(0,0,0,1)^T$ 是故意加入、且与 $A$ 列空间正交的不可拟合分量。

### 符号与对象账本

| 对象 | 本例中的值 | 回答的问题 |
|---|---|---|
| $A$ | $[\Sigma Q^T;0]$ | 设计矩阵怎样作用于参数？ |
| $x_\star$ | $(1,-1,2)^T$ | 精确最小二乘参数是什么？ |
| $r_\star$ | $b-Ax_\star=e_4$ | 哪部分目标无法由列空间表达？ |
| $\sigma(A)$ | $1,1/2,1/4$ | 各参数方向被放大/压缩多少？ |
| $G$ | $A^TA=Q\operatorname{diag}(1,1/4,1/16)Q^T$ | 正规方程看见什么谱？ |
| $u_i$ | $Qe_i$ | 三个右奇异/Gram 特征方向 |
| $\kappa_2(A)$ | $4$ | 原始最小二乘映射的尺度比 |
| $\kappa_2(G)$ | $16$ | 形成正规矩阵后的尺度比 |
...
$$
r_\star=b-Ax_\star=e_4,
\qquad
A^Tr_\star=0.
$$

$x_\star$ 满足正规方程，且 $A$ 满列秩，因此它是唯一最小二乘解；最小残差范数为 $1$。这个非零 residual 不是算法失败，而是 $b$ 中存在列空间无法表达的分量。
...
$$
U=\begin{bmatrix}I_3\\0_{1\times3}\end{bmatrix}
$$

看成具有标准正交列的 $4\times3$ 矩阵，则
...
$$
\kappa_2(A)=\frac{\sigma_1}{\sigma_3}
=\frac1{1/4}=4.
$$

沿弱方向 $u_3=Qe_3$，参数变化 $\alpha u_3$ 只在预测空间产生长度 $|\alpha|/4$ 的变化；小预测差异可能对应更明显的参数差异。

### 第三步：正规方程把每个奇异值再乘一次自身

$$
\begin{aligned}
G=A^TA
&=Q\Sigma U^TU\Sigma Q^T\\
&=Q\Sigma^2Q^T\\
&=Q\operatorname{diag}\!\left(1,\frac14,\frac1{16}\right)Q^T.
\end{aligned}
$$
...
弱方向 $u_3$ 在 $A$ 中的尺度是 $1/4$，在 $G$ 中只剩 $1/16$。显式形成 $A^TA$ 还会先对每个矩阵元素执行 dot-product reduction，因此可能在求解开始前就把弱谱信息舍入掉。

### 第四步：三条路径的数学目标相同，数值证据不同

| 路径 | 精确算术 | 中间条件尺度 | 主要交付 |
|---|---|---:|---|
| 正规方程 | 解 $Gx=A^Tb$ | $16$ | 成本低，但弱方向与数值秩最易受损 |
| Householder QR | 解 $Rx=Q_A^Tb$ | 约 $4$ | 保留原始列尺度，满秩稠密基线 |
| SVD/QRCP | 过滤奇异方向 | 显式暴露 $1,1/2,1/4$ | rank、最小范数、截断/正则化接口 |
...
并同时报告参数敏感性或数值秩。只报告 residual $\approx1$ 无法判断弱方向参数是否正确。

### 核心公式七问：$\kappa_2(A^TA)=\kappa_2(A)^2$

1. **从哪来？** $A=U\Sigma V^T$ 导出 $A^TA=V\Sigma^2V^T$。
2. **需要什么条件？** $A$ 满列秩，使用二范数条件数；秩亏时两者都为无穷。
3. **平方了什么？** 最大与最小非零奇异值之比，而不是矩阵每个元素。
4. **为何危险？** 弱方向的相对分辨率再损失一次，且形成 Gram 本身包含易相消点积。
5. **QR 为什么不同？** 正交变换不形成 $A^TA$，三角因子保留 $A$ 的奇异值尺度。
6. **正则化只是换算法吗？** 不是；ridge/TSVD 改变估计目标或可识别方向，参数必须进入任务合同。
7. **AI 中如何用？** 线性 probe、Gauss–Newton、LoRA 子空间拟合和 representation regression 都应区分 prediction residual、parameter error 与 numerical rank。

> [!warning] 教学模型边界
> 本例的奇异值刻意分离且矩阵很小，没有展示近秩亏时 rank decision 的不连续性、稀疏 QR fill 或大规模迭代 least squares。实际任务还包含数据噪声；此时“恢复真参数”本身可能没有统计可识别性，不能把所有误差归给求解器。

> [!success] 第一遍停靠线
> 应能验证 $A^Tr_\star=0$、写出薄 SVD、算出 $\kappa_2(A)=4$ 与 $\kappa_2(A^TA)=16$，并解释弱方向为什么从 $1/4$ 进一步变成 $1/16$。还要能说出 residual、stationarity residual 和 parameter sensitivity 分别回答什么。
