...
[[阶段测验 - 数值计算与数值线性代数（10.8）]]已经把 NUM-01—20 压缩为一张 180 分钟、100 分的累计测验：20 分定义与条件、30 分手算、25 分主推导、15 分失败诊断、10 分 AI 迁移，并附随机实验复现门。[[阶段测验解答 - 数值计算与数值线性代数（10.8）]]独立保存完整推导、评分断点和按失分区回链的补修路线。
