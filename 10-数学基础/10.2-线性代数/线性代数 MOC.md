...
| 验收件 | 覆盖与作用 | 当前状态 |
|---|---|---|
| [[阶段测验 - 线性代数（10.2）]] | 240 分钟、100 分，A—E 分区覆盖 LA-01—24 | composed / not-attempted |
| [[阶段测验解答 - 线性代数（10.2）]] | 逐题独立详解、评分边界、AI 迁移与错题回链 | sealed until first attempt |
| [[实验 - 线性代数累计复现门]] | 病态坐标/商与投影、Jordan/SVD、attention rank/vec 三轨 | composed / not-attempted |

`L2-A / L2-B / L2-C / LA-AI`保留为复习与补救分组，不再作为遗漏的独立卷；LA-CUM-01已经统一验收24个正式节点。工具、解答与SVG存在只证明验收链可执行，不代表学习者已经掌握，因此所有节点继续保持`draft`，直至首次闭卷、评分、随机轨道、48小时重做与14天迁移均有证据。
...
- 10.2 卷级验收已建立：[[阶段测验 - 线性代数（10.2）]]、[[阶段测验解答 - 线性代数（10.2）]]与[[实验 - 线性代数累计复现门]]已把24节点接成统一闭环；当前为`composed / not-attempted`；
