...
[[learning_theory_teaching_contract_audit.py]]已对固定主干执行独立静态与生成回归：LT-01—84 的 ID、路径和十卷数量完全闭合；84 组习题与独立解答形成双射，1260 个 A—E 题解 ID 完全一致；144 张实际调用来源卡均可定位，其中 `verified / active / draft = 13 / 129 / 2`，两张 `draft` 均为带断言审计和适用边界的科学空间 C 级桥梁，不承担正式定理主证据；2154 条作用域内 Wiki 链接、96 个节点嵌图/97 个章节图文单元和 84 个课程表映射通过。`--run-compute`又将 18 个节点制图脚本运行两次，当前 118 个已存学习理论资产（含卷级与资格考图）与仓库版本保持字节一致。
...
第四卷现已建立[[阶段测验 - 数据依赖复杂度、间隔与快率（20.4）|RAD-CUM-01]]、[[阶段测验解答 - 数据依赖复杂度、间隔与快率（20.4）|独立封存详解]]与[[实验 - 数据依赖复杂度、间隔与快率累计复现门|三轨累计复现门]]。它以八层数据依赖复杂度账本串联 ghost sample、经验 Rademacher、contraction、双范数线性类、margin 共同选择、cover/chaining、local fixed point 与 fat profile；`attempt_id + scorer nonce`、跨轨盲参、48 小时换几何/损失和 14 天陌生模型迁移把“复杂度更小”压成可审计的随机对象—几何传递—概率事件链。[[rademacher_margin_local_cumulative_contract_audit.py]]独立复核 8/8 scope、14/14 题解与 100 分、exact signs、margin 共同预算、internal covers、fixed point/fat profile、canonical/固定盲参双跑、SVG/XML/hash、覆盖/输入保护与六处状态面。

20.4 的**卷级验收材料**为 `regression-passed`，但个人仍为 `not-attempted`。至此是“已建立 4/10 材料门”，个人通过仍为 0/10。

### 20.5 卷级材料门

第五卷现已建立[[阶段测验 - 稳定性、压缩、PAC-Bayes 与信息泛化（20.5）|ALG-CUM-01]]、[[阶段测验解答 - 稳定性、压缩、PAC-Bayes 与信息泛化（20.5）|独立封存详解]]与[[实验 - 稳定性、压缩、PAC-Bayes 与信息泛化累计复现门|三轨累计复现门]]。它以八层算法依赖泛化账本串联 ghost replacement、RERM/SGD stability、fixed sample compression、PAC-Bayes-kl 测度变换、prior/posterior 时间线、mutual-information channel 与五路线选择审计；`attempt_id + scorer nonce`、跨轨盲参、48 小时换机制与 14 天陌生 AI 迁移防止事后挑证书。[[algorithmic_generalization_cumulative_contract_audit.py]]独立复核 8/8 scope、14/14 题解与 100 分、exact replace-one/Binomial gap、compression/KL/inverse-kl、binary-channel MI、canonical/固定盲参双跑、SVG/XML/hash、support/覆盖/非法输入保护与六处状态面。

20.5 的**卷级验收材料**为 `regression-passed`，但个人仍为 `not-attempted`。至此是“已建立 5/10 材料门”，个人通过仍为 0/10。

### LT-QUAL-01 跨卷资格考材料门

前五卷现在由[[资格考 - 学习理论 I：从风险到算法依赖泛化（20.1—20.5）|LT-QUAL-01]]统一验收。它不是五卷题目的拼接，而是让同一个 finite threshold learner 同时经过 object/risk、finite/VC、Rademacher、stability、compression、PAC-Bayes 与 information 七条语言；30 分钟口试、两场共 300 分钟闭卷、十二层账本、scorer nonce、跨轨 blind、非法合同、48 小时换机制和 14 天陌生论文路由共同防止公式识别式通过。

配套[[资格考解答 - 学习理论 I：从风险到算法依赖泛化（20.1—20.5）|独立封存详解]]逐题分离 predictor、sample、event 与 statement type；[[实验 - 学习理论资格考 I 跨卷累计复现门]]用 all-sample/all-replacement exact track、large-sample certificate track 与 route ledger track 复核跨卷接缝；[[learning_theory_qualification_01_contract_audit.py]]不导入生成函数，独立重算 output law、ghost、stability、entropy/MI、finite/compression/PAC-Bayes canonical 与 blind 锚点，并检查覆盖保护和非法合同。

因此 `LT-QUAL-01` 的**资格考材料**为 `regression-passed`；它建成时跨卷资格考材料门为 **1/2**。没有五卷 retained、闭卷原稿、个人 blind 与延迟证据，个人资格仍为 **0/2 / `not-attempted`**。随后课程进入 20.6，而不是把材料建成误写成已经掌握前五卷。

### 20.6 卷级材料门

第六卷现已建立[[阶段测验 - 经典模型与模型选择（20.6）|MODEL-CUM-01]]、[[阶段测验解答 - 经典模型与模型选择（20.6）|独立封存详解]]与[[实验 - 经典模型与模型选择累计复现门|三轨累计复现门]]。它用十层经典模型账本连接 estimand、model/loss、algorithm、selection、refit 与 deployment；三轨分别精确核对 spectral ridge/validation、margin/partition/ensemble、PCA/K-Means/EM/identifiability，而不是把十二种算法写成 API 问答。

[[classical_models_cumulative_contract_audit.py]]独立复算 canonical 与跨轨 blind 数学锚点，检查 12/12 节点、14/14 题解、100 分、双跑、SVG/XML/hash、非法输入、`LT-QUAL-01` 前置和六处状态面。因此 20.6 的**卷级验收材料**为 `regression-passed`，全章卷级材料门达到 **6/10**；个人仍为 **0/10 / `not-attempted`**，且 `LT-QUAL-01 retained` 的正式前置尚未满足。下一施工点进入 20.7。

### 20.7 卷级材料门

第七卷现已建立[[阶段测验 - 表示学习、度量学习与自监督（20.7）|REPR-CUM-01]]、[[阶段测验解答 - 表示学习、度量学习与自监督（20.7）|独立封存详解]]与[[实验 - 表示学习、度量学习与自监督累计复现门|三轨累计复现门]]。它用七层表示学习账本连接 source unit、view/target law、encoder/head、objective、selection、downstream protocol 与 deployment；三轨分别精确核对 task-indexed risk/metric/dependent views、candidate-index InfoNCE/batch/collision、non-collapse/VICReg/EMA/masked target，而不是把八个节点写成 SSL 方法表。

[[representation_selfsupervised_cumulative_contract_audit.py]]独立复算 canonical 与跨轨 blind 数学锚点，检查 8/8 节点、14/14 题解、100 分、双跑、SVG/XML/hash、非法输入、`MODEL-CUM-01` 前置和六处状态面。因此 20.7 的**卷级验收材料**为 `regression-passed`，全章卷级材料门达到 **7/10**；个人仍为 **0/10 / `not-attempted`**，且 `MODEL-CUM-01 retained` 的正式前置尚未满足。下一施工点进入 20.8。

### 20.8 卷级材料门

第八卷现已建立[[阶段测验 - 校准、不确定性与分布偏移（20.8）|REL-CUM-01]]、[[阶段测验解答 - 校准、不确定性与分布偏移（20.8）|独立封存详解]]与[[实验 - 校准、不确定性与分布偏移累计复现门|三轨累计复现门]]。它用八层可靠性账本分离 joint law、predictive object、identifying assumption、certificate、selection、decision utility、shift set 与 evidence boundary；三轨分别精确核对 calibration/proper scores/predictive variance、conformal rank/importance weighting/overlap、domain discrepancy/OOD threshold/group risk，而不把所有目标混成一个“更可靠”。

[[calibration_shift_cumulative_contract_audit.py]]不导入生成函数，独立复算 Brier 分解、mixture total variance、split-conformal 顺序统计量、covariate-shift identity/ESS、$\mathcal H\Delta\mathcal H$ discrepancy、OOD AUROC 和 worst-group risk，并检查 8/8 节点、14/14 题解、100 分、canonical/固定 blind 双跑、SVG/XML/hash、非法合同、`REPR-CUM-01` 前置和六处状态面。因此 20.8 的**卷级验收材料**为 `regression-passed`，全章卷级材料门达到 **8/10**；个人仍为 **0/10 / `not-attempted`**，且 `REPR-CUM-01 retained` 的正式前置尚未满足。下一施工点进入 20.9。

### 20.9 卷级材料门

第九卷现已建立[[阶段测验 - 在线学习、Boosting 与序列预测（20.9）|ONLINE-CUM-01]]、[[阶段测验解答 - 在线学习、Boosting 与序列预测（20.9）|独立封存详解]]与[[实验 - 在线学习、Boosting 与序列预测累计复现门|三轨累计复现门]]。它用八层序列学习账本分离 move order、filtration、feedback、comparator、potential、probability、conversion 与 deployment boundary；三轨分别精确核对 Hedge/OGD/current-action adversary、Perceptron/AdaBoost、online-to-batch/UCB/IPS/RL boundary。

[[online_boosting_cumulative_contract_audit.py]]不导入生成函数，独立复算 log-weight/distance potential、progress/norm/$\prod Z_t$、random-iterate risk/UCB/IPS variance，并检查 8/8 节点、14/14 题解、100 分、canonical/固定 blind 双跑、SVG/XML/hash、非法合同、`REL-CUM-01` 前置和六处状态面。因此 20.9 的**卷级验收材料**为 `regression-passed`，全章卷级材料门达到 **9/10**；个人仍为 **0/10 / `not-attempted`**，且 `REL-CUM-01 retained` 的正式前置尚未满足。下一施工点进入 20.10。

### 20.10 卷级材料门

第十卷现已建立[[阶段测验 - 深度泛化理论接口与开放边界（20.10）|DEEP-CUM-01]]、[[阶段测验解答 - 深度泛化理论接口与开放边界（20.10）|独立封存详解]]与[[实验 - 深度泛化理论接口与开放边界累计复现门|三轨累计复现门]]。它用九层证据账本分离 function、data law、algorithm、parameterization、proxy、regime、quantifier、intervention 与 deployment；三轨分别精确核对 interpolation risk/min-norm/null-space selection、sharpness rescaling/norm capacity、fixed-kernel modes/finite-particle feature drift。

[[deep_generalization_cumulative_contract_audit.py]]不导入生成函数，独立复算双下降风险、min-norm/null shift、sharpness/capacity、kernel eigendynamics 与 particle drift，并检查 8/8 节点、14/14 题解、100 分、canonical/固定 blind 双跑、SVG/XML/hash、非法合同、`ONLINE-CUM-01` 前置和六处状态面。因此 20.10 的**卷级验收材料**为 `regression-passed`，全章卷级材料门达到 **10/10**；个人仍为 **0/10 / `not-attempted`**，且 `ONLINE-CUM-01 retained` 的正式前置尚未满足。

### LT-QUAL-02 跨卷资格考材料门

后五卷现在由[[资格考 - 学习理论 II：从模型选择到深度泛化证据（20.6—20.10）|LT-QUAL-02]]统一验收。它不是五张卷级试卷的拼接，而是让同一个 source→target→online 二分类服务依次经过模型/表示选择、概率/校准/shift、full-information regret、context-action OPE 与深度机制证据审计；35 分钟口试、两场共 320 分钟闭卷、十三层账本、scorer nonce、跨轨 blind、非法合同、48 小时换 pipeline 与 14 天陌生 claim card 共同阻止对象偷换。

配套[[资格考解答 - 学习理论 II：从模型选择到深度泛化证据（20.6—20.10）|独立封存详解]]逐题分离 source/target winner、deterministic model/stochastic policy、regret/risk/value 与 training dynamics/population theorem；[[实验 - 学习理论资格考 II 跨卷累计复现门]]用 source-target representation、online-OPE、interpolation-invariance-regime 三轨复核跨卷接缝；[[learning_theory_qualification_02_contract_audit.py]]不导入生成函数，独立重算 Brier/ESS、Hedge/joint-ratio IPS、min-norm/sharpness/kernel/particle canonical 与 blind 锚点。

因此 `LT-QUAL-02` 的**资格考材料**为 `regression-passed`，学习理论两份跨卷资格考材料门达到 **2/2**；没有 `LT-QUAL-01 + 后五卷 retained`、两场原稿、个人 blind 与延迟迁移证据，个人资格仍为 **0/2 / `not-attempted`**。材料建设完成后的下一步是按前置顺序执行个人证据，而不是继续增加平行题卷。
...
| 已建立卷级证据门 | 10 / 10 | 20.1—20.10 十份卷级材料全部回归通过 |
| 已通过分卷 | 0 / 10 | 尚无个人口试、闭卷、盲参和延迟证据 |
| 已建立跨卷资格考材料 | 2 / 2 | LT-QUAL-01 与 LT-QUAL-02 分别覆盖 20.1—20.5、20.6—20.10；题卷、详解、三轨实验与独立审计均已回归通过 |
| 已通过跨卷资格考 | 0 / 2 | 尚无五卷 retained 前置、个人闭卷、blind 与延迟迁移证据；状态为 `not-attempted` |
| 下一施工点 | 个人证据执行 | 按前置顺序完成分卷 retained、两次资格考闭卷、blind、48 小时重做与 14 天迁移 |
