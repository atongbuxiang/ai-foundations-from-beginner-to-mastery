...
因此本卷的**验收材料**为 `regression-passed`，但学习者仍为 `not-attempted`，不得把 8/8 材料闭环写成个人通过。本门建成时把全章卷级材料推进到 **5/10**。

前五卷的[[资格考 - 学习理论 I：从风险到算法依赖泛化（20.1—20.5）|LT-QUAL-01]]、独立详解与[[实验 - 学习理论资格考 I 跨卷累计复现门|三轨累计复现门]]现已建立；后五卷也已由[[资格考 - 学习理论 II：从模型选择到深度泛化证据（20.6—20.10）|LT-QUAL-02]]闭合。全章当前为 **10/10 卷级材料门、2/2 资格考材料门**，个人仍为 **0/10、0/2 / `not-attempted`**；下一步是按前置顺序执行个人证据。
