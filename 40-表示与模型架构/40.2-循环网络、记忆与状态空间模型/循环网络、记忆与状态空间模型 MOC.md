...
当前为 **8/8 现行教学迁移、8/8 A—E 题解闭环、8/8 `regression-passed` material**；全章累计为 **ARCH-01—16（16/64）**，分卷材料门为 **2/8**。两波以 $\mathcal S_\square$ 贯通状态碰撞、BPTT、LSTM/GRU、ZOH、recurrence/convolution/scan、HiPPO/S4 与 selective path，并由[[architecture_teaching_contract_audit.py]]独立复算八幅正式图和精确数值。个人学习状态为 `not-attempted`，下一施工点为 ARCH-17—20。
