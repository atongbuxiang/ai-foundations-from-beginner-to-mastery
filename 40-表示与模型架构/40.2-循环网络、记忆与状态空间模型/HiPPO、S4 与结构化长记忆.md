...
## 导读：在讨论“长记忆”以前，先说明究竟要记住什么

说一个状态模型“能够记住很长的历史”听起来很有吸引力，但这句话缺少一个关键对象：它要以什么误差标准记住历史的哪些部分？逐 token 原样回放、估计长期平均、保留最近变化趋势和预测下一个值，是四种不同的记忆任务。HiPPO 的重要之处，是先把这个问题写成函数空间中的投影，而不是先凭经验选择一个循环矩阵。

在这个视角下，状态的每一维不再是一个神秘的 memory slot，而是历史函数沿某个基方向的系数。选择测度，就是决定过去不同位置的重要性；选择有限基，就是决定状态愿意保留哪类模式。正交投影保证的是：在这个已经选定的有限子空间里，当前重构误差最小。它并不保证任意历史都能被有限状态无损恢复。

S4 接着解决另一个问题：即使这样的状态动力学有良好的记忆解释，长卷积核怎样才能高效生成和应用？HiPPO 矩阵具有可利用的 normal/diagonal-plus-low-rank 结构，使大的 resolvent 计算可以还原为逐元素逆和小规模低秩修正。本节会用二维投影和一个 $2\times2$ rank-one inverse 分别说明“记什么”与“怎么算”，并且始终不把这两层证据混在一起。
...
这组投影公式回答的是表示问题，而不是完整的 S4 实现问题。接下来我们先证明状态坐标为何对应最佳有限维近似，再讨论这些坐标怎样形成 ODE；只有到这一步以后，DPLR、resolvent 和长卷积的计算结构才有明确对象可以作用。
