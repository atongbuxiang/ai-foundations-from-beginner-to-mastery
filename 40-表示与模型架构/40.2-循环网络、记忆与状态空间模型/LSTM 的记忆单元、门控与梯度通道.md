...
## 导读：如果每一步都覆盖旧记忆，怎样才能记得更久

在 vanilla RNN 中，新状态通常由一次 affine 变换和非线性整体重写。这样做很灵活，却让长期信息每一步都必须穿过新的矩阵和激活函数。LSTM 的出发点不是简单地“多加几个门”，而是把长期存储和对外暴露分成两个状态角色：$c_t$ 负责内部累积，$h_t$ 负责把当前可用的信息送给下一层或输出头。

你可以把一次 cell 更新拆成三个问题。旧内容要保留多少？新候选要写入多少？内部内容现在要向外展示多少？forget、input 和 output gate 分别回答这三个问题。这里的门是逐维连续系数，不是一次只能开或关的布尔开关，也不是必须相加为 1 的概率分配。

真正改变梯度路径的是加法更新。只沿显式的 $c_{t-1}\to c_t$ 边看，导数是 forget gate 的乘积；当这些值接近 1 时，这条路径可以接近恒等映射。但请不要提前把它读成“LSTM 已经解决梯度消失”：门本身依赖过去 hidden，完整 Jacobian 还包含许多间接路径。本节会先固定门值看清直接通道，再把完整计算图放回来。
...
这两个方程很紧凑，却同时描述了存储和接口。若流式推理时只保存 $h_t$ 而丢掉 $c_t$，运行的已经不是同一个 LSTM；若只观察 forget 乘积而忽略门网络的间接导数，分析的也只是总梯度的一条路径。下面先声明本课程采用的现代 LSTM 方程，再逐个解释门、参数量和梯度边界。
