...
ARCH-09 只计算了状态怎样向前传播，现在把问题反过来问：如果最后一步预测错了，第一步的输入和共享循环权重应该承担多少责任？这不是再写一个新的学习规则，而是把已经展开的递推计算图倒着走一遍。困难在于，同一个早期状态不仅影响当前输出，还会改变所有未来状态，所以它收到的反向信号必须把这些路径全部加起来。

我们先不碰复杂矩阵。沿用上一节的标量线性递推，末端损失从 $h_3$ 开始，连续乘三次 $1/2$ 回到 $h_0$。这个例子会同时揭示两个常被混淆的对象：状态梯度沿时间递推，而共享参数梯度要把参数在每个时间使用点的贡献相加。只计算最后一次使用，并不是 BPTT。

等标量链清楚以后，再把 $1/2$ 换成随时间和轨迹改变的 Jacobian。此时“梯度消失或爆炸”不再只是某个特征值大于还是小于 1，而是一个有序矩阵乘积怎样在不同方向上伸缩。非线性饱和、非正规瞬态和时变方向都会参与其中，所以单独看 $W_{hh}$ 的谱半径只能给出非常有限的线索。
...
这条伴随递推就是普通反向传播在时间展开图上的写法。它没有神秘的新微积分，真正困难的是路径很长且参数重复使用。下面我们先把 RNN 的完整 shape 写清楚，再区分状态伴随、激活前伴随与参数梯度；这样进入 LSTM 时，你能准确看见门控究竟改写了哪一条梯度路径。
