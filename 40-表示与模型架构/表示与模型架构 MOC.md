...
> [!info] 当前教学迁移快照
> ARCH-01—24 已按当前“课程位置—两遍路线—问题链—对象账本—贯穿算例—公式七问”标准完成六波迁移，现行范围为 **24/64**；40.1、40.2 与 40.3 均为 **8/8 `regression-passed`**，分卷材料门推进至 **3/8**。节点正文继续保留 `draft`，个人学习严格保持 `not-attempted`。
...
| [[图表示与消息传递神经网络 MOC]] | ARCH-17—24 | 无固定顺序的邻接结构怎样被等变处理？ | 8/8 `regression-passed` material |
...
- 当前教学标准迁移：ARCH-01—24 为 24/64，ARCH-25—64 尚待按初学者问题链重审；
- 40.1、40.2 与 40.3 当前均为 8/8 `regression-passed`，全章材料门推进至 3/8；40.4—40.8 的旧静态材料不等于现行教学迁移完成；
- 第四章累计题卷、独立解答与跨卷确定性复现门已组成，但尚未真实作答；
- 分卷材料门：3/8；真实学习验收：0/64，个人状态 `not-attempted`。
