...
> [!info] 当前教学迁移快照
> ARCH-01—04 已按当前“课程位置—两遍路线—问题链—对象账本—贯穿算例—公式七问”标准完成首波迁移，现行范围为 **4/64**；40.1 为 **4/8 in-progress**，分卷材料门仍为 **0/8**。节点正文继续保留 `draft`，个人学习严格保持 `not-attempted`。
...
| 卷 | 稳定 ID | 核心问题 | 状态 |
|---|---|---|---|
| [[卷积、空间结构与等变性 MOC]] | ARCH-01—08 | 局部连接和参数共享怎样编码空间先验？ | ARCH-01—04 当前标准；4/8 in-progress |
| [[循环网络、记忆与状态空间模型 MOC]] | ARCH-09—16 | 有限状态怎样压缩历史，何时能并行？ | 静态材料完成 |
...
- 当前教学标准迁移：ARCH-01—04 为 4/64，ARCH-05—64 尚待按初学者问题链重审；
- 40.1 当前为 4/8 in-progress，40.1—40.8 的旧静态材料不等于现行教学迁移完成；
- 第四章累计题卷、独立解答与跨卷确定性复现门已组成，但尚未真实作答；
- 分卷材料门：0/8；真实学习验收：0/64，个人状态 `not-attempted`。
