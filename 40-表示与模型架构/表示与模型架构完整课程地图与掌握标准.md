...
| ID | 核心节点 | 层级 | 核心问题 | 状态 |
|---|---|---|---|---|
| ARCH-09 | [[序列因果性、隐藏状态与递推计算]] | 核心 | 固定维状态怎样压缩可变长历史？ | regression-passed material / learner `not-attempted` |
| ARCH-10 | [[Vanilla RNN、BPTT 与梯度消失爆炸]] | 核心 | 共享递推 Jacobian 为什么造成长程优化困难？ | regression-passed material / learner `not-attempted` |
| ARCH-11 | [[LSTM 的记忆单元、门控与梯度通道]] | 核心 | additive cell state 如何缓解但不消除长程问题？ | regression-passed material / learner `not-attempted` |
| ARCH-12 | [[GRU、门控递推与 RNN 结构比较]] | 核心 | update/reset gate 与 LSTM 的状态合同有何差异？ | regression-passed material / learner `not-attempted` |
| ARCH-13 | [[连续与离散线性状态空间模型]] | 核心 | $\dot x=Ax+Bu$ 怎样离散成稳定 recurrence？ | regression-passed material / learner `not-attempted` |
| ARCH-14 | [[状态空间的递推—卷积对偶与并行扫描]] | 进阶 | 何时可把线性 recurrence 改写成卷积或 scan？ | regression-passed material / learner `not-attempted` |
| ARCH-15 | [[HiPPO、S4 与结构化长记忆]] | 进阶 | 正交投影怎样导出状态矩阵，S4 如何高效计算？ | regression-passed material / learner `not-attempted` |
| ARCH-16 | [[选择性状态空间、Mamba 与证据边界]] | 进阶 | 输入依赖参数带来何种选择性，又牺牲什么并行结构？ | regression-passed material / learner `not-attempted` |
...
| ID | 核心节点 | 层级 | 核心问题 | 状态 |
|---|---|---|---|---|
| ARCH-17 | [[图数据、节点重标号与置换对称性]] | 核心 | 邻接和特征怎样随节点 permutation 共同变换？ | regression-passed material / learner `not-attempted` |
| ARCH-18 | [[消息传递神经网络的统一形式]] | 核心 | message、aggregate、update 三步如何保持等变？ | regression-passed material / learner `not-attempted` |
| ARCH-19 | [[谱图卷积、空间图卷积与归一化邻接]] | 核心 | Laplacian 频域与邻居聚合如何相连？ | regression-passed material / learner `not-attempted` |
| ARCH-20 | [[聚合器、可辨识性与 Graph Isomorphism Network]] | 进阶 | sum/mean/max 对多重集分别丢失什么？ | regression-passed material / learner `not-attempted` |
| ARCH-21 | [[图网络深度、过平滑与过挤压]] | 进阶 | 多跳信息为何会同质化或压过窄瓶颈？ | regression-passed material / learner `not-attempted` |
| ARCH-22 | [[图注意力与结构偏置]] | 核心 | 学习邻居权重与全局 Attention 有何同异？ | regression-passed material / learner `not-attempted` |
| ARCH-23 | [[图级读出、异构图与任务接口]] | 核心 | 节点表示怎样成为 permutation-invariant 图表示？ | regression-passed material / learner `not-attempted` |
| ARCH-24 | [[WL 表达界、反例与 GNN 证据地图]] | 进阶 | message passing 能区分哪些图，基准证据怎样读？ | regression-passed material / learner `not-attempted` |
...
> ARCH-01—24 已完成六波教学重写并由[[architecture_teaching_contract_audit.py]]独立复核，迁移范围为 **24/64**；40.1、40.2 与 40.3 均为 **8/8 `regression-passed`**，分卷材料门推进至 **3/8**。正文 `draft` 与个人 `not-attempted` 不变，旧“静态材料完成”只说明文件、题解和图存在。
...
| 现行教学迁移 | 24 / 64 | ARCH-01—24 已满足当前初学者教学合同 |
| 分卷验收门 | 3 / 8 | 40.1、40.2、40.3 已回归通过；下一施工点为 ARCH-25—32 |
