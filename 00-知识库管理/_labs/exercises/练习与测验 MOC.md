...
每套均包含 A–E 分层训练与独立解答。神经网络基础 NN-01—64 已有 64 组 960 题，并已组成 NN-CUM-01；学习理论 LT-01—84 共 84 组 1260 题；MATH当前完成6组90题；早期底座 D0 新增 60 题；几何—求解 D0-G 新增 45 题；谱—低秩 D0-S 新增 75 题；矩阵度量—敏感性 D0-N 新增 60 题；标准空间结构 D1 新增 45 题；D2/D3/D4分别新增30/30/45题。CALC十六组共240题并已归入CALC-CUM-01；PROB二十组共300题；INFO十组共150题；OPT十六组共240题；DYN十二组共180题。线性代数复习桶进入LA/MA累计卷，数值三组进入NLA-CUM-01。
...
| LA-CUM-01 | LA-01—24：线性代数卷末累计验收 | composed：[[阶段测验 - 线性代数（10.2）]]、[[阶段测验解答 - 线性代数（10.2）]]和[[实验 - 线性代数累计复现门]]已建立；等待真实独立作答 |
...
| NLA-CUM-01 | NUM-01—20：数值计算与数值线性代数卷末累计验收 | composed：[[阶段测验 - 数值计算与数值线性代数（10.8）]]、[[阶段测验解答 - 数值计算与数值线性代数（10.8）]]和实验复现门已建立；等待真实独立作答 |
...
LA-CUM-01是240分钟、100分的10.2卷末测验，覆盖LA-01—24，并设置A—E分区线、三道统一结构证明不得为零和随机三轨计算门。当前状态是 **composed / not-attempted**：

- 24套节点题之后的累计题卷与逐题独立详解已经成稿；
- 计算门连接病态basis/quotient/projector、Jordan transient/SVD tail与attention-softmax rank/vec identity；
...
CALC-CUM-01是270分钟、100分的10.4卷末测验，覆盖CALC-01—16，并设置A—E分区线、三道统一证明不得为零和随机三轨计算门。当前状态是 **composed / not-attempted**：

- 16套节点题、累计题卷与逐题独立详解已经成稿；
- 计算门连接Taylor/finite-difference、JVP/VJP/HVP与implicit/spectral derivative；
- canonical SVG、XML、hash、确定性运行与实际渲染通过，只证明工具可执行；
- 尚无首次闭卷原稿、评分、参数干预、48小时重做和14天AI迁移证据；
...
NLA-CUM-01 已把 L3-A、L3-B 与 L3-C 的题库组合成 100 分闭卷卷末测验，覆盖 NUM-01—20，并另设随机指定的实验复现门。当前状态是 **composed / not-attempted**：

- 题卷和独立详解成稿，只证明“已有可执行验收工具”；
- 实验是组合门：从[[实验 - 条件估计、误差传播与可信停止]]、[[实验 - 稳定归约、点积消去与混合精度累加]]、[[实验 - 三精度迭代改进与 GMRES-IR 边界]]中由评分者随机指定一项；
- 尚无学习者原始答卷、逐项评分、48 小时重做和 14 天迁移证据；
- 因此不把任何节点或 10.8 分卷升级为 verified；
- 正式作答前不要打开[[阶段测验解答 - 数值计算与数值线性代数（10.8）]]。

### PROB-CUM-01 的状态边界

PROB-CUM-01 是 180 分钟、100 分的闭卷卷末测验，覆盖 PROB-01—20，并设置 A—E 分区线、三道主链不得为零和随机指定计算轨道。当前状态为 **composed / not-attempted**：

- 题卷、独立详解与 coverage–IS–MCMC 三轨计算门已经成稿；
- 文档与脚本通过只证明验收工具可执行，不证明学习者掌握；
- 尚无首次闭卷原稿、逐题评分、48 小时重做、14 天迁移与随机轨道复现证据；
