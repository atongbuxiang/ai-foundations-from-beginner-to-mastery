...
每套均包含 A–E 分层训练与独立解答。神经网络基础已有 64 组 960 题；[[neural_network_foundations_teaching_contract_audit.py]]确认 NN-01—64 现行迁移 **64/64**、待迁移 **0/64**、30.1—30.8 材料门 **8/8 `regression-passed`**。[[neural_network_foundations_cumulative_contract_audit.py]]进一步确认 NN-CUM-01 的 14/14 题解、100 分、口试/隔离/nonce/盲参/延迟门为材料 `regression-passed`；个人保持 `not-attempted`。
...
| ARCH-CAP-01 | ARCH-01—64：表示与模型架构第四章跨卷累计验收 | composed：ARCH-01—20 已完成 20/64 现行教学迁移，40.1—40.2 材料门 2/8 `regression-passed`，40.3 为 4/8 迁移中，并由[[architecture_teaching_contract_audit.py]]复核；全章累计门尚未升级，个人 `not-attempted` |
