...
def audit_state_surfaces() -> None:
    ...
    for content, label in ((root_moc, "root MOC"), (curriculum, "curriculum map")):
        require(re.search(r"8\s*/\s*10", content) is not None, f"{label}: cumulative volume-gate count not synchronized")
        require(re.search(r"0\s*/\s*10", content) is not None, f"{label}: personal volume count changed")
    print(f"PASS state surfaces: {len(STATE_SURFACES)} views preserve LT-CUM-01 and agree on 8/10 material gates, 0/10 personal passes")
