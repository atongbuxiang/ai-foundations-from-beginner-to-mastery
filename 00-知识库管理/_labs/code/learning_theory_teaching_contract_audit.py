...
def audit_state_surfaces() -> None:
    ...
    for path in STATE_SURFACES:
        ...
        require(re.search(r"6\s*/\s*10", content) is not None, f"state surface misses six built volume gates: {path.relative_to(ROOT)}")
    ...
    print(f"PASS LT state surfaces: {len(STATE_SURFACES)} views agree on 6/10 material gates and 0/10 personal passes")
