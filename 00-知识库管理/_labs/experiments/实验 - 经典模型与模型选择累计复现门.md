...
> A 轨以 diagonal fixed-design model 精确计算 ridge bias/variance/df，并枚举全部验证噪声；B 轨在六点分类问题上计算 separation、最大间隔、Gini split、全部 $6^6$ bootstrap 与一轮 AdaBoost；C 轨手算 covariance spectrum、K-Means distortion、对称 mixture EM 和声明清楚的 AIC/BIC arithmetic。图由 [[classical_models_cumulative_gate.py]] 确定性生成，无 Monte Carlo、无第三方依赖。
