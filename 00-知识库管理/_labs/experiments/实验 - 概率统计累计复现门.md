...
计算门必须沿用题卷的同一 `attempt_id`，并严格按下列顺序执行：
...
> [!warning] 答案与输出隔离
> canonical run 只校准环境，不能代替盲干预。新参数 output 出现前必须冻结预测；详解出现前必须冻结 output。任何换 nonce、换轨、删 seed/chain 或用仓库现成 SVG 冒充个人运行，都会使本次证据失效。
...
### 0.3 scorer nonce：不可挑轨

评分者先写一个未公开选择偏好的 nonce。把本次 `attempt_id` 与 nonce 原样拼接，由以下规则唯一指定轨道：

```bash
python3 -c 'import hashlib; s="PROB-CUM-01-YYYYMMDD-A|SCORER-NONCE"; print("ABC"[int(hashlib.sha256(s.encode()).hexdigest()[:8],16)%3])'
```

证据中同时保存原字符串、完整 SHA-256 和输出轨道。评分者只指定**深入手检轨**；学习者仍须运行 A/B/C 合并图。若规则实现错误，应保留原记录并重新开始新的 attempt，而不是试 nonce 直到得到喜欢的轨道。
...
3. 修改到 near-boundary $p$ 前预注册“Wald coverage 将明显恶化”，再运行验证；
...
正式个人验收由评分者给出一组**未列在本文中的参数**。下面的组合仅是仓库回归夹具，用来证明接口和审计能工作；学习者不得把它当个人盲题。

### 8.1 三轨干预与预测合同

| 轨 | 可干预参数 | 运行前必须写的方向预测 |
|---|---|---|
| A | `--coverage-p`、`--coverage-reps` | near-boundary 时 Wald coverage 尤其在小 $n$ 恶化；重复数主要改变 coverage Monte Carlo SE，不修复 procedure 本身 |
| B | `--tail-threshold`、`--narrow-sigma`、`--wide-sigma`、draws/repeats | threshold 增大使真值与普通 MC 命中下降；更宽 proposal 可改善该 tail functional，但 ESS 未必更大 |
| C | `--mode-location`、`--proposal-step`、warmup/draws | mode separation 增大且局部 step 变小时跨峰更难；同峰链仍可能 $\widehat R\approx1$，分散链会强烈报警 |

每条预测都要说明：改变的是 DGP、estimand、algorithm 还是 simulation budget；预期哪个数值上升/下降；若结果冲突，优先检查哪一个假设。

### 8.2 仓库回归用组合参数

```bash
python3 "00-知识库管理/_labs/code/plot_probability_cumulative_gate.py" \
  --coverage-p 0.1 --coverage-reps 2500 \
  --tail-threshold 3.5 --narrow-sigma 0.8 --wide-sigma 2.5 \
  --is-repeats 30 --is-draws 15000 \
  --mode-location 7 --proposal-step 1.2 --warmup 800 --mcmc-draws 3000 \
  --output "/tmp/probability-gate-intervention.svg"
```

该夹具的 SHA-256 应为 `31402ed50a404dcd9444353f8d3a1155d928b34f968061b3c77b1f27a0064b75`。典型机制信号是：$p=0.1$ 时 $n=20,50$ 的 Wald coverage 约为 $0.883,0.894$；threshold $3.5$ 时 $\sigma=0.8$ 的 30 次运行全为零命中而 $\sigma=2.5$ relative RMSE 约 $0.043$；mode 在 $\pm7$ 时，同左峰 $\widehat R\approx1.002$ 但 mean $\approx-7$，分散初始化 $\widehat R\approx7.63$。

这些固定数值只用于回归，不是需要背诵的理论。正式盲题应核对**方向、对象和失败解释**，浮点末位仅在同一环境/种子下用于排查实现漂移。
...
stateDiagram-v2
    ...
    corrected --> retained_48h: 换机制无提示重建
    retained_48h --> transferred_14d: 陌生AI研究迁移
    transferred_14d --> passed: 独立评分通过
```

`corrected` 只说明已理解反馈，不等于独立保持。48 小时任务必须换 DGP/coupling、functional、selection/stopping、prior/model 或 sampler geometry 中至少两项；14 天任务必须在陌生 AI 摘要中重新辨认生成机制、estimand、重复单位、三层不确定性与失败停止规则。
...
> 题卷、详解、脚本、canonical SVG、盲干预接口和独立审计通过后，本材料记为 `material_status: regression-passed`。仓库没有替学习者执行任何一次真实 attempt，因此 `learning_status: not-attempted` 保持不变；只有完整证据链和独立评分才能升级个人状态。
