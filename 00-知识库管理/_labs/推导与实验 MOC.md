...
- 第一资格考：[[实验 - 学习理论资格考 I 跨卷累计复现门]]以 all-sample/all-replacement exact learner、large-sample finite/compression/PAC-Bayes certificates 与 predictor/event route ledger 组成三轨；配套[[资格考 - 学习理论 I：从风险到算法依赖泛化（20.1—20.5）|LT-QUAL-01]]和独立详解；
...
- 第八卷累计门：[[实验 - 校准、不确定性与分布偏移累计复现门]]以 calibration/proper score/predictive mixture、conformal rank/importance weighting/overlap、domain discrepancy/OOD threshold/group risk 组成三轨；配套[[阶段测验 - 校准、不确定性与分布偏移（20.8）|REL-CUM-01]]和独立详解；
- [[calibration_shift_cumulative_contract_audit.py]]：独立复算 8/8 节点、14/14 题解、calibration/coverage/shift 锚点、canonical/跨轨 blind、SVG/XML/hash、非法合同、20.7 前置与六处状态面；
- 第九卷累计门：[[实验 - 在线学习、Boosting 与序列预测累计复现门]]以 Hedge/OGD/adversary visibility、Perceptron/AdaBoost、online-to-batch/UCB/IPS 组成三轨；配套[[阶段测验 - 在线学习、Boosting 与序列预测（20.9）|ONLINE-CUM-01]]和独立详解；
...
- 第八波 NN-29—32 延伸 $\mathcal I_\square$：从 ReLU correlation map 走到半正交局部 Jacobian 的 rank-2 反例，再用同一 hidden slice 闭合 zero-head/skip 梯度、LSUV 数据校准与 Fixup 深度缩放；
...
- [[00-知识库管理/_labs/code/architecture_teaching_contract_audit.py]]：现行教学迁移审计；当前覆盖 ARCH-01—12（12/64），独立复算 convolution/equivariance、aliasing/anti-alias、RF/jump、CNN budget、$C_4$ lifting、state collision、BPTT finite difference 与 LSTM/GRU 门控账，并双跑十二幅迁移内正式 SVG；40.1 材料门为 1/8 `regression-passed`、40.2 前半卷 4/8 `in-progress`，个人状态保持 `not-attempted`；
...
- 当前语义：ARCH-01—12 为 12/64 现行教学迁移，40.1 为 8/8 `regression-passed`、40.2 为 4/8 `in-progress`，材料门 1/8；脚本断言只验证确定性 toy constructions，不替代真实训练、学习验收或论文复现，个人仍为 `not-attempted`。
...
- 计算门：[[实验 - 数值线性代数累计复现门]]以 `attempt_id + scorer nonce` 指定 A 有限精度/误差/停止、B 结构/Krylov/预条件或 C 稀疏成本/随机低秩深入轨；要求两项手算、盲参预测、新 output/SVG/hash、48 小时换机制与 14 天陌生 AI 数值迁移；独立回归见[[numerical_cumulative_contract_audit.py]]；
- 覆盖：NUM-01—20，15 分钟口试、180 分钟 100 分闭卷、nonce 随机轨、盲参数与延迟迁移门；
- 当前语义：材料 `regression-passed`、个人 `not-attempted`，不构成 verified 证据。
