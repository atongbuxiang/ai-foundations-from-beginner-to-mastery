...
related: ["[[数值线性代数 MOC]]", "[[数学基础完整课程地图与掌握标准]]", "[[练习与测验 MOC]]", "[[推导与实验 MOC]]", "[[实验 - 数值线性代数累计复现门]]"]
created: 2026-08-15
updated: 2026-08-28
---
...
每次正式尝试先建立 `attempt_id = NLA-CUM-01-YYYYMMDD-A`，再依次完成：

1. 15 分钟无提示口试；
2. 180 分钟闭卷笔试，到时冻结原稿；
3. 评分者公布 `scorer nonce`，指定 A/B/C 手算轨与多参数盲干预；
4. 学习者先提交解析校准、方向/数值预测与失败标准，再生成新 stdout、SVG 和 SHA-256；
5. 所有原始证据冻结后才能打开解答与 canonical 输出订正；
6. 48 小时后做换机制重建，14 天后处理陌生 AI 数值系统；
7. 最后运行独立累计审计，只根据实际证据更新学习状态。

`material_status: regression-passed` 与 `learning_status: not-attempted` 必须分开保存：前者说明仓库材料可执行，后者说明尚无本人通过证据。

### 0.2 八层数值对象账本

| 层 | 必须先写的对象 | 常见越界 |
|---|---|---|
| 1 数学任务 | solution、residual、eigen/subspace、rank-$k$ approximation、downstream gradient | 把输出张量当成任务证书 |
| 2 数据与结构 | shape、rank、SPD/symmetric/nonnormal/rectangular/sparse/matrix-free | 未验证结构就选 CG/MINRES |
| 3 算术合同 | storage/multiply/accumulate/output/residual precision、rounding、FMA/FTZ | 只写“混合精度” |
| 4 误差来源 | data、rounding、backward、forward、iteration、truncation、random sketch | 把所有误差压成一个 residual |
| 5 算法与表示 | factorization、projection/Krylov、preconditioner、restart、sparse format | 只报算法名或 iteration count |
| 6 证书 | true residual、BERR/FERR、condition estimate、orthogonality、probability | 把 recurrence diagnostic 当 final certificate |
| 7 成本 | flops、bytes、matvec/VJP、synchronization、fill、passes、seeds | 把 `nnz` 或 wall-clock 当普适成本 |
| 8 AI 验收 | task tolerance、gradient quality、latency/memory budget、fallback | 数值收敛直接推出部署成功 |

### 0.3 五波统一模型族

- **A 波**：$\mathbb F_{10,4}$ 与 $A_\tau$ 建立舍入—residual—condition—task gate 链；
- **B 波**：$A_\varepsilon$ 串起 reduction、pivoted solve、mixed-precision refinement 与 stable QR；
- **C 波**：$A\to G=A^TA\to T$ 串起 least squares、power/RQI、QR eigenvalue 与 Lanczos；
- **D 波**：非正规 $A$ 串起 Arnoldi、SVD、Richardson transient 与 preconditioning；
- **E 波**：同一稀疏算子上用 CG/GMRES、CSR 和 randomized range finder 形成终端证书。

### 0.4 答案与输出隔离协议

正式尝试期间，[[阶段测验解答 - 数值计算与数值线性代数（10.8）|解答]]、实验结果节、canonical stdout/hash 和旧尝试记录均必须隔离。`scorer nonce` 在笔试冻结后才公布；不得先看结果再选简单轨道、dtype、步长或稀疏格式。预测文件与输出文件必须共享同一 `attempt_id`。
...
- 开始笔试前先完成第九节 15 分钟卷级口试；口试不计入 100 分，但必须通过；
- 笔试后完成第十节不计分但必须通过的[[实验 - 数值线性代数累计复现门|累计复现门]]。
...
4. 第 12 题的数值判断不得为 0；
5. 卷级口试通过；
6. 实验复现门的 nonce 随机手算轨与多参数盲干预同时通过；
7. 48 小时换机制重建与 14 天陌生 AI 数值迁移通过。

> [!warning] 状态语义
> 本测验、详解、canonical/盲参脚本和独立审计达到 `material_status: regression-passed`，不会让任何章节自动升级。只有真实独立口试、闭卷作答、nonce 随机轨、盲干预、评分、错题回链、48 小时换机制和 14 天迁移都留下证据，`learning_status` 才可从 `not-attempted` 逐级升级。
...
口试在闭卷笔试之前进行，不允许看图、笔记或提示。四问按 `3 + 4 + 4 + 4` 分钟控制；评分者可以追问一个“为什么”和一个“如果条件不成立会怎样”。

### 口试 1：五波模型链（必答）

从 $\mathbb F_{10,4}$ 与 $A_\tau$ 出发，用不超过三分钟讲清 NUM-01—20 的五波模型链。不能只报章节名；每一波至少要说出：研究对象、一个核心公式/证书、它怎样把问题交给下一波。
...
区分并串起：problem conditioning、algorithmic stability、residual、forward error 与 downstream task error。回答必须包含一个“小 residual 但大 forward error”的具体例子，并说明稳定算法为什么仍不能治愈病态问题。

### 口试 3：按结构选方法并写最终证书

对下列任务快速选择方法，给出合法性条件和最终验收量：

1. 超定矩形最小二乘；
2. SPD 线性系统；
3. 对称不定系统；
4. 一般非正规隐式算子；
5. 只需大矩阵的近似 rank-$k$ 子空间。

只报 QR、CG、MINRES、GMRES、randomized SVD 名称不算完成；必须补上 residual/orthogonality、结构检查、成本或失败概率。

### 口试 4：AI 数值研究合同（必答）

为“只能用 VJP 的隐式层反向线性系统”口头设计一份最小合同，覆盖：算子和形状、结构验证、dtype/累加层级、预条件器、递推与真实 residual、条件/任务容差、回退状态、成本与梯度质量日志。最后说出至少两个“看起来成功但必须拒绝”的状态。

### 口试通过线与红线

- 四问至少通过三问；第 1、4 问必须通过；
- 每问必须同时有对象、关系、条件和失败边界，背诵算法清单不算通过；
- 把“小 residual”等同于“小 forward error”、把“$\rho(B)<1$”等同于“每步单调下降”、把“CSR/随机”直接等同于“更快/有保证”，任一误读未能在追问后纠正，该问不通过；
- 口试未通过时，先回链第一处断点，48 小时后换例重做，不用笔试高分覆盖概念混淆。
...
## 十、实验复现门（必须通过，不计入 100 分）

完成[[实验 - 数值线性代数累计复现门]]。先运行三轨总图，再由评分者在下列轨道中随机指定一轨深入复核，不能由学习者挑选：

1. A 轨：有限精度—误差—稳定—停止；
2. B 轨：结构—投影—预条件—真 residual；
3. C 轨：稀疏成本—随机值域—独立证书。

通过条件：

- 从代码入口重新生成 canonical SVG，记录环境、命令和 SHA-256；
- 在不看终端输出时手工复核指定轨道至少两个数值；
- 修改该轨控制参数，在运行前写预测，干预图写到临时路径而不覆盖 canonical 图；
- 明确写出“直接观测—理论桥梁—任务结论—不可推出结论”；
- 把证书映射到一个 AI 任务容差、显存、data pass 或失败回退合同，并保留与预测冲突的结果。
...
48 小时后不得只把 $\tau$ 或步长换一个数重跑原图。评分者从下列五类中指定一类，学习者在 45 分钟内重建“对象→结构→误差源→算法→证书→任务门”：

- 换 arithmetic/dtype/reduction path；
- 换 matrix structure、right-hand side 或 norm；
- 换 factorization/Krylov/restart/preconditioner；
- 换 sparse format/index width/data-pass budget；
- 换 randomized sketch/probe/seed/failure probability 或 downstream task tolerance。

通过要求：至少重新推导一条误差或成本关系，给出一个能否定当前方案的失败例，并说明原 theorem/algorithm contract 中哪一条不再保留。

## 十三、14 天陌生 AI 数值迁移门

评分者提供一份未见的 AI 数值系统摘要，例如隐式层、二阶/HVP 子问题、大型 embedding/attention 低秩近似、图稀疏算子或多设备混合精度求解。报告必须包含：

1. operator/data/shape 与实际输出对象；
2. structure 检查与算法选择合法性；
3. storage/multiply/accumulate/residual 精度和误差来源分账；
4. true residual、BERR/FERR、condition/orthogonality/probability 中的最终证书；
5. flops/bytes/matvec/VJP/synchronization/passes/seeds 成本；
6. downstream tolerance、latency/memory budget、fallback 和至少三个 failure criteria。
...
scorer nonce 与指定手算轨：
盲参命令、事前预测与失败标准：
canonical / blind stdout、SVG 与 SHA-256：
订正中的第一个断点与回链：
48 小时换机制证据：
14 天陌生 AI 数值报告：
独立审计 stdout 与时间：
...
口试、笔试、nonce 指定手算和盲参预测冻结前不要打开：[[阶段测验解答 - 数值计算与数值线性代数（10.8）]]。打开后的补算、重跑与改写均标记为 `corrected`，不得倒填为 independent。
