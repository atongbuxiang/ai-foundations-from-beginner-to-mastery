...
每次正式尝试先建立唯一编号 `attempt_id = OPT-CUM-01-YYYYMMDD-A`，然后按以下顺序留存证据：

1. 15 分钟无提示口试；
2. 210 分钟闭卷笔试，到时立即冻结原稿；
3. 评分者公布 `scorer nonce`，用 nonce 指定一条手算轨和一组盲参数；
4. 先提交预测、证明与失败标准，再生成新 SVG、stdout 和 SHA-256；
5. 上传上述冻结证据后，才能打开解答与 canonical 输出订正；
6. 48 小时后完成换机制题，14 天后完成一个陌生 AI 优化任务；
7. 最后运行独立审计，同步材料状态与个人学习状态。

`material_status: regression-passed` 只回答“题卷、解答、实验和脚本是否自洽”；`learning_status: not-attempted` 仍回答“学习者是否真的提交了证据”。两者不得用同一个 `status` 代替。

### 0.2 九层优化对象账本

遇到任何新题，先写账本，后选算法。若某层题目未给出，必须标记“未指定”，不得默认补齐。

| 层 | 先写什么 | 必须分开的概念 |
|---|---|---|
| 1 问题合同 | variables、data、domain、objective、constraints、solution set | training objective / deployment target |
| 2 几何 | norm、inner product、metric、coordinate chart | Euclidean / $H$-metric / Bregman / Fisher |
| 3 正则性 | convexity、smoothness、strong convexity、PL、nonconvexity | 条件之间只按定理推理 |
| 4 oracle | exact/stochastic gradient、HVP、prox、projection | unbiasedness / variance / solve error |
| 5 动力学 | state、update、step size、stopping rule | iterate / average / best iterate |
| 6 约束与算子 | tangent/normal、CQ、KKT、dual、prox | necessary / sufficient / lower bound |
| 7 证书 | gap、residual、rate、noise floor、confidence interval | diagnostic / certificate |
| 8 参数化 | symmetry orbit、quotient/function-space quantity | coordinate sharpness / predictor invariant |
| 9 经验与部署 | compute、tuning、seed、held-out/OOD、latency | empirical / population / deployment |

### 0.3 四波统一模型族

- **A 波**：三角形上的 projection/Fenchel 问题，建立“问题对象—几何—全局证书”；
- **B 波**：$H=\operatorname{diag}(1,4)$ 的谱二次型，加入 GD/HB/SGD 动力学与噪声平台；
- **C 波**：加入线性项与三角形 constraint，比较 Newton、不同 metric projection 与 KKT residual；
- **D 波**：从同一 convex core 进入 dual/prox/mirror/Fisher，再以 $w=ab$ 暴露 saddle、symmetry 与 raw sharpness 的参数化边界。

### 0.4 答案与输出隔离协议

正式尝试期间，[[阶段测验解答 - 优化与凸分析（10.7）|解答]]、实验结果节、canonical stdout/hash 与旧尝试记录均必须隔离。`scorer nonce` 由评分者在笔试冻结后给出；学习者不得先看输出再选容易的轨道或参数。盲参预测文件和新输出必须都带同一 `attempt_id`。
...
4. 计算复现门的 nonce 随机轨和盲参数干预同时通过；
5. 48 小时后无提示完成换机制重建，14 天后完成一道未见过的优化建模与算法选择题；
6. 独立累计审计通过，但不得用材料回归代替前五项个人证据。

> [!warning] 状态语义
> 题卷、解答、盲参回归和审计已达 `regression-passed`，只说明教学材料可执行。在没有真实答卷、评分、错题回链和延迟复测之前，16 个节点保持 `draft`，个人卷级状态保持 `not-attempted`，不得批量升级为 mastered。
...
计算门不计入 100 分，但是整卷通过的必要条件。在笔试冻结后，评分者用 `scorer nonce` 完成两次指定：

1. 在 A/B/C 三轨中随机抽一轨，要求不看代码重建核心等式、预测方向并写一条不可推出结论；
2. 在 `--perturbation`、`--stable-y0`、`--eta`、`--steps`、`--pl-a`、`--pl-x-max`、`--scale-span` 中指定至少三项，要求先冻结预测，再运行到新文件；
3. 提交完整命令、stdout、SVG、SHA-256、Python 版本与预测偏差解释；
4. canonical 和盲干预必须具有不同输出路径，不得覆盖仓库中的标准产物。

详细参数合同、预测表和记录模板见[[实验 - 优化与凸分析累计复现门]]。

## 十二、48 小时换机制重建门

48 小时后不得只换一个数字重跑原题。评分者从下列五类中抽取一类，学习者在 45 分钟内重建“对象→假设→算法→证书→边界”：

- 换 objective/constraint/CQ；
- 换 norm、metric 或 parameterization；
- 换 oracle/noise model；
- 换 algorithm、step rule 或 inner-solve tolerance；
- 换 certificate/deployment quantity。

通过要求：新问题合同完整，至少有一条重新推导的等式、一个反例/失败条件，且不把原问题的 theorem 无条件搬过来。

## 十三、14 天陌生 AI 优化迁移门

评分者提供一份未见过的 AI 优化摘要，例如带 latency/fairness 约束的训练、低秩适配器、对抗内层问题、mixture routing 或大模型分布式优化。报告必须回答：

1. 什么是变量，什么在本层固定；objective 与 deployment estimand 是什么；
2. 问题使用哪种 geometry/oracle，所引 theorem 的完整合同是什么；
3. 为什么选该算法，更新或子问题如何执行；
4. 哪些 residual/gap 是 certificate，哪些只是 diagnostic；
5. 如何控制 compute/tuning/seed、symmetry/parameterization 与 held-out/OOD/deployment 证据；
6. 哪三个现象一出现就否定当前结论。
...
scorer nonce 与随机指定轨：
盲参命令、事前预测与失败标准：
canonical / blind SVG 路径与 SHA-256：
订正中的第一个断点与回链：
48 小时换机制题与评分：
14 天陌生 AI 迁移报告与评分：
独立审计 stdout 与时间：
...
只有在口试、笔试、随机指定与盲参预测已冻结后，才打开[[阶段测验解答 - 优化与凸分析（10.7）]]。打开解答后的评注、补算和重跑必须标记为订正，不得混入原答。
