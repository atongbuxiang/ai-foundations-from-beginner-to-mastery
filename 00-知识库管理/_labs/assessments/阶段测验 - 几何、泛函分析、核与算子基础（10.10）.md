...
执行前建立唯一 `attempt_id`，建议格式为 `GEO-CUM-01-YYYYMMDD-A`。同一次尝试的口试记录、闭卷原稿、评分表、终端输出、干预图和复测结果都使用同一 ID；一旦打开详解，不得覆盖或润色第一次答案。

> [!important] 材料状态与个人状态分开
> `material_status: regression-passed` 只表示题卷、详解、脚本、图和交叉审计自洽；`learning_status: not-attempted` 才是当前个人证据。第一次真实作答只能从 `not-attempted` 进入 `attempted`，不能因材料完善而跳到 `passed-initial` 或 `retained`。
...
| 局部线性化 | $T_pM$、$T_p^*M$、$df_p$、metric 与 gradient | sphere tangent、Exp、retraction | 把 coordinate/ambient vector 当内蕴对象 |
...
| 作用对象 | projection、bounded/compact operator、kernel/solution operator | $P_m$、diagonal $K$、Green operator $\mathcal G$ | 把有限矩阵低秩当连续 compactness |
| 证据接口 | sample、Gram matrix、basis cutoff、mesh/quadrature、error topology | KRR、Galerkin、Poisson modal cutoff | 把点残差或小绝对误差当 operator 泛化 |

### 0.2 三波如何在一条主链上接合

1. **第一波 GEO-01—04：**先问空间里什么叫“附近”，再问局部可用哪个线性空间计算，最后问变换怎样同时作用于输入、参数和输出；
2. **第二波 GEO-05—06：**把向量从 $\mathbb R^d$ 升级为完备函数/序列空间，再把矩阵升级为需要单独审计 boundedness、compactness 与 spectrum 的 operator；
3. **第三波 GEO-07—08：**用 Hilbert geometry 解释 kernel representation，再把同一 Green kernel 解释为 weak PDE 的 solution operator，最终把连续定理接到有限样本、有限基与有限网格；
...
口试失败时仍可做闭卷作为诊断，但本次最多记 `needs-remediation`，不能记 `passed-initial`。
...
- `passed-initial`：口试、闭卷与随机实验通过，但延迟门尚未完成；
- `retained`：48小时换例与14天陌生迁移均独立通过；
- `verified-candidate`：仅表示证据可送入GEO-01—08逐节点审查，不能自动批量把八篇正文改成 `verified`。
...
| A—E分区分数与总分 |  |
| 第9—11题主证明状态 |  |
| 随机实验轨道、scorer nonce 与 canonical hash |  |
| 干预前预测、输出路径与偏差解释 |  |
| 第一个断点与GEO正文回链 |  |
| 48小时换例重做 |  |
| 14天陌生迁移题 |  |
